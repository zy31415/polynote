from tests.polynote_testkit.node_op import create_note, sync_and_assert_notes_equal


def test_update_conflicts_b_has_latest_update(node_a, node_b):
    created = create_note(node_a)
...
def test_update_conflicts_tie_breaker(node_a, node_b):
    ...
    node_a.update_note(note_id, ts=3, title="Update from A", body="This is 2st update from node A")
    node_a.update_note(note_id, ts=4, title="Update from A", body="This is 2st update from node A")

    node_b.update_note(note_id, ts=1, title="Update from B", body="This is an update from node B")
    node_b.update_note(note_id, ts=4, title="Update from B", body="This is an update from node B")
...
def test_update_conflicts_a_has_latest_update(node_a, node_b):
    ...
    node_a.update_note(note_id, ts=1, title="Update from A", body="This is 1st update from node A")
    node_a.update_note(note_id, ts=3, title="Update from A", body="This is 2st update from node A")
    node_a.update_note(note_id, ts=4, title="Update from A", body="This is 3rd update from node A")
    node_a.update_note(note_id, ts=5, title="Update from A", body="This is 3rd update from node A")
    # note lamport timestamp is 6 at node_a after the above updates

    node_b.update_note(note_id, ts=1, title="Update from B", body="This is an update from node B")
    node_b.update_note(note_id, ts=4, title="Update from B", body="This is an update from node B")
    # note lamport timestamp is 5 at node_b after the above updates
