import pytest
from polynote_testkit.node_op import create_a_note_then_update_then_delete
from tests.polynote_testkit.node_op import create_a_note_then_update, sync_and_assert_notes_equal, create_a_note


def test_create_note_replicates_between_2_nodes(node_a, node_b):
    create_a_note(node_a)
    create_a_note(node_b)
...
def test_create_and_update_note_replicates_between_2_nodes(node_a, node_b, update_count_a, update_count_b):
    create_a_note_then_update(node_a, update_count_a)
    create_a_note_then_update(node_b, update_count_b)
    sync_and_assert_notes_equal(node_a, node_b, expected_count=2)
...
def test_create_update_delete_note_replicates_between_2_nodes(node_a, node_b, update_count_a, update_count_b):
    create_a_note_then_update_then_delete(node_a, update_count_a)
    create_a_note_then_update_then_delete(node_b, update_count_b)
