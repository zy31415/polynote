...
def test_update_delete_conflict_delete_from_A_win(node_a, node_b):
    ...
    node_a.delete_note(note_id, ts=5)
    # note lamport timestamp is 6 at node_a after the above delete
    ...
    # note lamport timestamp is 5 at node_a after the above updates
...
def test_update_conflicts_tie_breaker_update_from_A_win(node_a, node_b):
    ...
    node_a.update_note(note_id, ts=1, title="Update from A", body="This is 1st update from node A")
    node_a.update_note(note_id, ts=3, title="Update from A", body="This is 2st update from node A")
    # note lamport timestamp is 4 at node_a after the above updates
